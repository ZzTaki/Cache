...
class ARCCache(object):
    def __init__(self, capacity):
        ...
        self.p = capacity / 2  # t1的最大空间，初始化t1和t2对半分
        self.hit_num = 0
        self.total_num = 0
    ...
    def visit(self, id, value):
        ...
        if id in self.t1 or id in self.t2:  # 命中
            ...
            self.p = min(
                self.p + max(self.b1[id], self.size_b2 //
            ...
            self.p = max(self.p - max(self.size_b1 //
                         self.size_b2, self.b2[id]), 0)
...
with open(file, "r", encoding="utf-8") as f:
    ...
    print("总请求数: " + str(cache.total_num) + ", 命中次数: " + str(cache.hit_num))
    print("-------ARC替换算法-------")
f.close()
